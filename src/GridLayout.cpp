...
void GridLayout::resizeNodeSizePos(SGridNodeData *node, int x, int y, int width,
                                   int height) {
...
void GridLayout::moveWindowToWorkspaceSilent(CWindow *pWindow,
  ...
  if (const auto PATCOORDS = g_pCompositor->vectorToWindowIdeal(OLDMIDDLE);
      PATCOORDS && PATCOORDS != pWindow)
...
void GridLayout::onWindowCreatedTiling(CWindow *pWindow, eDirection direction) {
  ...
  const auto PNODE =
      &m_lGridNodesData.emplace_back(); // make a new node in list back

  const auto PACTIVEWORKSPACE =
      g_pCompositor->getWorkspaceByID(PMONITOR->activeWorkspace);

  const auto PWINDOWORIWORKSPACE =
      g_pCompositor->getWorkspaceByID(pWindow->m_iWorkspaceID);

  PNODE->workspaceID =
      pWindow->m_iWorkspaceID; // encapsulate window objects as node objects to
  ...
  {
    if (PWINDOWORIWORKSPACE->m_iID != PACTIVEWORKSPACE->m_iID ||
...
bool GridLayout::isWindowTiled(CWindow *pWindow) { return false; }
...
void GridLayout::calculateWorkspace(const int &ws) {
  ...
  const auto PMONITOR =
      g_pCompositor->getMonitorFromID(PWORKSPACE->m_iMonitorID);

  if (NODECOUNT == 0)
    return;

  static const auto *PBORDERSIZE =
      &HyprlandAPI::getConfigValue(PHANDLE, "general:border_size")->intValue;
  static const auto *GAPPO =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hycov:overview_gappo")
           ->intValue;
  static const auto *GAPPI =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hycov:overview_gappi")
           ->intValue;
  ...
  if (NODECOUNT == 1) {
    ...
    resizeNodeSizePos(NODE, m_x + w_x + (int)((m_width - cw) / 2),
                      m_y + w_y + (int)((w_height - ch) / 2), cw - 2 * (*PBORDERSIZE),
                      ch - 2 * (*PBORDERSIZE));
    return;
  }
  ...
  if (NODECOUNT == 2) {
    ...
    resizeNodeSizePos(NODE, m_x + cw + (*GAPPO) + (*GAPPI),
                      m_y + (m_height - ch) / 2 + (*GAPPO),
                      cw - 2 * (*PBORDERSIZE), ch - 2 * (*PBORDERSIZE));
    resizeNodeSizePos(tempNodes[1], m_x + (*GAPPO),
                      m_y + (m_height - ch) / 2 + (*GAPPO),
                      cw - 2 * (*PBORDERSIZE), ch - 2 * (*PBORDERSIZE));
    return;
  }
  ...
  for (i = 0, NODE = tempNodes[0]; NODE; NODE = tempNodes[i + 1], i++) {
    ...
    resizeNodeSizePos(NODE, m_x + cx + (*GAPPO), m_y + cy + (*GAPPO),
                      cw - 2 * (*PBORDERSIZE), ch - 2 * (*PBORDERSIZE));
  }
}
...
void GridLayout::recalculateMonitor(const int &monid) {
  ...
  const auto PWORKSPACE = g_pCompositor->getWorkspaceByID(
      PMONITOR->activeWorkspace); // 获取当前workspace对象
  if (!PWORKSPACE)
    return;
...
void GridLayout::resizeActiveWindow(const Vector2D &pixResize,
                                    eRectCorner corner, CWindow *pWindow) {
  ; // empty
}

void GridLayout::fullscreenRequestForWindow(CWindow *pWindow,
                                            eFullscreenMode mode, bool on) {
  ; // empty
}

std::any GridLayout::layoutMessage(SLayoutMessageHeader header,
                                   std::string content) {
  return "";
}
...
std::string GridLayout::getLayoutName() { return "grid"; }
...
void GridLayout::changeToActivceSourceWorkspace() {
  ...
  const auto PWORKSPACE =
      g_pCompositor->getWorkspaceByID(Node->ovbk_pWindow_workspaceID);
...
void GridLayout::moveWindowToSourceWorkspace() {
  ...
  for (auto &nd : m_lGridNodesData) {
    if (nd.pWindow &&
        (nd.pWindow->m_iWorkspaceID != nd.ovbk_pWindow_workspaceID ||
         nd.workspaceName != nd.ovbk_pWindow_workspaceName)) {

      pWorkspace = g_pCompositor->getWorkspaceByID(nd.ovbk_pWindow_workspaceID);
      if (!pWorkspace)
        pWorkspace = g_pCompositor->createNewWorkspace(
            nd.ovbk_pWindow_workspaceID, nd.pWindow->m_iMonitorID,
            nd.ovbk_pWindow_workspaceName);
